...
class SigLoss(nn.Module):
    def __init__(self, config):
        ...
        self.fix_sig_loss_param = config.fix_sig_loss_param
        if self.fix_sig_loss_param:
            self.temp = math.exp(config.temperature)
            self.bias = config.bias
        else:
            self.temp_prior = nn.Parameter(torch.tensor([config.temperature]))
    ...
    def forward(self, sim_matrix):
        if self.fix_sig_loss_param:
            ...
            t = torch.exp(self.temp_prior)
        sim_matrix = t * sim_matrix + self.bias
        n = sim_matrix.shape[0]
        label = 2 * torch.eye(n, device=sim_matrix.device) - 1
        logpt = torch.log(torch.sigmoid(sim_matrix * label))
        nce_loss = -logpt
        sim_loss = nce_loss.sum() / n
        return sim_loss
