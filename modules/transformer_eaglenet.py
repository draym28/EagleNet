...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, text_embeds, video_embeds):
        """
        Input
            text_embeds: num_texts x num_vids x embed_dim
            video_embeds: num_vids x num_frames x embed_dim
        Output
            o: num_vids x num_texts x embed_dim
        """
        num_texts, num_vids, _ = text_embeds.shape
        # num_texts x num_vids x embed_dim
        q = self.q_proj(text_embeds)
        q = q.reshape(num_texts, num_vids, self.num_heads, self.head_dim)
        ...
        attention_logits = torch.einsum('bfhd,abhd->abfh', [k, q])  # (num_texts, num_vids, num_frames, num_heads)
        attention_logits = attention_logits.permute(1,3,2,0)  # (num_vids, num_heads, num_frames, num_texts)
...
class Transformer(nn.Module):
    def __init__(self, hdim, heads, dropout):
        super(Transformer, self).__init__()
        self.embed_dim = hdim
        heads = heads
        dropout = dropout

        self.cross_attn = MultiHeadedAttention(self.embed_dim, heads)
